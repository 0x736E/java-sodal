...
/*
 * Description: Base IrcBot class
 * Notes: incomplete, need to expand on events and add features.
...
// TODO: Test the channels/modHasChannels methods
// its very important to ensure its secure.
...
public class IrcBot extends PircBot implements Serializable {
	...
	private String[][][]	modChannels;		// module | [channel | password ]
	private boolean[]		modsUseAllChannels;	// list of modules which use all the channels the bot is connected to
	
	private IrcFilter[] 	objFilters;
	private String[][][]	filterChannels;		// filter | [ channel | password ]
	private boolean[]		filtUseAllChannels;	// list of filters which may use all the channels the bot is connected to
	
	private String[][]		channelCache;		// need to keep a temporary cache of channels for the 'use all channels' functionality
	
	private boolean			verbose				= false;		// _verbose from pircbot is not visible :(
	...
	{
		// get rid of all the filters
    	for( int i = 0; i < objFilters.length; i++ )
    		objFilters[i].dispose();
	...
	{
		// parent class does not expose
		// the verbose variable. unfortunately.
		verbose = verb;
	...
	public final int addMod( IrcModule mod )
	{
		...
		temp[newLen - 1] = mod;							// assign the object
		temp[newLen - 1].botIndex = (newLen - 1);		// tell the mod which index it was loaded at
		objMods = temp;
	
		// add a new array index to contain the channels for this mod
		String[][][] tmp = { { { null, null } } };
		modChannels = Arrays.appendArray(modChannels, tmp);
		
		// add a new array index to contain the boolean value for this mod
		// Default setting for new mods: use all bot channels
	...
	public final void removeMod( int index )			// TODO: shrink modChannels & modsUseAllChannels
	{
	...
	{
		for(int i=0; i<objMods.length; i++) {
			if(objMods[i].getName().equals(name))
				return objMods[i];
		}
		return null;
	}
	...
	// mod channels : the channels modules are applied to
	// TODO: add a 'removeModChannel' method
	...
	public final void addModChannel( int index, String chan, String pass ) {
		if(modChannels[index] == null) {
			...
			String[][] out = { { chan, pass } };
			modChannels[index] = Arrays.appendArray(modChannels[index], out);		// just added, does it work?
		}
	}
	public final void addModChannels( int index, String[][] chanArray )
	{
		...
		{
			for(int i=0; i<chanArray.length; i++) {
				for(int j=0; j<modChannels.length; j++)
				{
					// only add unique channels (dont join channels we've already joined)
					if(!modChannels[index][j][0].toLowerCase().equals(chanArray[i][0]))
					{
						String[][] temp = { { chanArray[j][0], chanArray[j][1] } };
						modChannels[index] = Arrays.appendArray(chanArray, temp);
					}
				}
			}	
		}
		...
		{
			modChannels[index] = chanArray;	// should check for uniqueness			
		}
		*/
	}
	...
	public final boolean modHasChannel( int index, String chan ) { 
		// use bot channels list
		if(modsUseAllChannels[index])
		{
			if(channelCache == null || channelCache.length <= 0) {	// TODO: check if length<=0 is necessary; i don't think it is
				return false;
			} else {
				for(int i=0; i<channelCache.length; i++) {
					if(channelCache[i][0] != null && channelCache[i][0].equals(chan))
						return true;
				}
			}
		}
		// use module channels list
		else
		{
			if(modChannels == null || modChannels.length <= 0) {
				...
				for(int i=0; i<modChannels[index].length; i++) {
					if(modChannels[index][i][0] != null && modChannels[index][i][0].equalsIgnoreCase(chan))
						return true;
				}
			}
		}
	...
	public final void removeFilter( int index )			// TODO: shrink filterChannels & filtUseAllChannels
	{
		String str = "unspecified";
		IrcFilter[] temp = new IrcFilter[ objFilters.length - 1 ];
	...
	{
		for(int i=0; i<objFilters.length; i++) {
			if(objFilters[i].getName().equals(name))
				return objFilters[i];
		}
		return null;
	}
	...
	// filter Channels : the channels filters are applied to
	...
	public final void addFilterChannel( int index, String chan, String pass ) {
		if(filterChannels[index] == null) {
			...
			String[][] out = { { chan, pass } };
			filterChannels[index] = Arrays.appendArray(filterChannels[index], out);		// just added, does it work?
		}
	}
	...
	public final boolean filterHasChannel( int index, String chan ) { 
		...
		{
			if(getChannels() == null || getChannels().length <= 0) {	// TODO: check if length<=0 is necessary; i don't think it is
				return false;
			} else {
				for(int i=0; i<getChannels().length; i++) {
					if(getChannels()[i].equalsIgnoreCase(chan))
						return true;
				}
			}
		}
		...
		else
		{
			if(filterChannels == null || filterChannels.length <= 0) {
				...
				for(int i=0; i<filterChannels[index].length; i++) {
					if(filterChannels[index][i][0] != null && filterChannels[index][i][0].equalsIgnoreCase(chan))
						return true;
				}
			}
		}
    ...
    public final void onConnectionReset()
    {
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status)
    			objMods[i].onConnectionReset();
    	}
    }
    
    // [modified] PircBot actions ////////
    // provided for module accessibility//

    public final void SendAction(String target, String action)
    {
    	if(!filterOverride && !modsOverride)
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status)
    			objMods[i].onSendAction(target, action);
    	}
    }
    public final void SendMessage(String target, String message)
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status)
    			objMods[i].onSendMessage(target, message);
    	}
    }
    public final void SendNotice(String target, String notice) 
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status)
    			objMods[i].onSendNotice(target, notice);
    	}
    }
    ...
    {
    	// I want to capture logs
    	// and offer the option to output to
    	// console, disk or both.
    	
    	// hour:minutes:seconds.miliseconds
    	// 10:12:13.14
    	if(verbose)
    		System.out.println("[" + Calendar.HOUR + ":" + Calendar.MINUTE + ":" + Calendar.SECOND + "." + Calendar.MILLISECOND + "] " +  line);
    ...
    public synchronized void dispose()
    {
    	...
    	// finally, dispose of the irc bot
    	super.dispose();
    	
    	// last of all, exit the JVM		
		System.exit(0);
    ...
    public final void onAction(String sender, String login, String hostname, String target, String action)
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status)
    			objMods[i].onAction(sender, login, hostname, target, action);
    	}
    }   
    @Override
    public final void onChannelInfo(String channel, int userCount, String topic)
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status && modHasChannel(i,channel))
    			objMods[i].onChannelInfo(channel, userCount, topic);
    	}
    }
    @Override
	...
	public final void onDccChatRequest(String sourceNick, String sourceLogin, String sourceHostname, long address, int port)
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status)
    			objMods[i].onDccChatRequest(sourceNick, sourceLogin, sourceHostname, address, port);
    	}
    } 
    @SuppressWarnings("deprecation")
	@Override
	public final void onDccSendRequest(String sourceNick, String sourceLogin, String sourceHostname, String filename, long address, int port, int size)
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status)
    			objMods[i].onDccSendRequest(sourceNick, sourceLogin, sourceHostname, filename, address, port, size);
    	}
    }
    @Override
    public final void onDeop(String channel, String sourceNick, String sourceLogin, String sourceHostname, String recipient)
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status && modHasChannel(i,channel))
    			objMods[i].onDeop(channel, sourceNick, sourceLogin, sourceHostname, recipient);
    	}
    }
    @Override
    public final void onDeVoice(String channel, String sourceNick, String sourceLogin, String sourceHostname, String recipient) 
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status && modHasChannel(i,channel))
    			objMods[i].onDeVoice(channel, sourceNick, sourceLogin, sourceHostname, recipient);
    	}
    }
    @Override
    ...
    public final void onFileTransferFinished(DccFileTransfer transfer, Exception e)
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status)
    			objMods[i].onFileTransferFinished(transfer, e);
    	}
    }
    @Override
    public final void onFinger(String sourceNick, String sourceLogin, String sourceHostname, String target) 
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status)
    			objMods[i].onFinger(sourceNick, sourceLogin, sourceHostname, target);
    	}
    }
    @Override
    public final void onIncomingChatRequest(DccChat chat) 
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status)
    			objMods[i].onIncomingChatRequest(chat) ;
    	}
    }
    @Override
    public final void onIncomingFileTransfer(DccFileTransfer transfer) 
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status)
    			objMods[i].onIncomingFileTransfer(transfer) ;
    	}
    }
    @Override
    public final void onInvite(String targetNick, String sourceNick, String sourceLogin, String sourceHostname, String channel) 
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status && modHasChannel(i,channel))
    			objMods[i].onInvite(targetNick, sourceNick, sourceLogin, sourceHostname, channel);
    	}
    }
    @Override
    public final void onJoin(String channel, String sender, String login, String hostname)
    {
    	// TODO: add channel to array
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status && modHasChannel(i,channel))
    			objMods[i].onJoin(channel, sender, login, hostname);
    	}
    }
    @Override
    ...
    {    	
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status && modHasChannel(i,channel))
    			objMods[i].onKick(channel, kickerNick, kickerLogin, kickerHostname, recipientNick, reason);
    	}
    	
    	// TODO: subtract channel from channelCache
    ...
    public final void onMessage(String channel, String sender,String login, String hostname, String message) 
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status && modHasChannel(i,channel))
        		objMods[i].onMessage(channel, sender, login, hostname, message);
    	}
    }
    @Override
    public final void onMode(String channel, String sourceNick, String sourceLogin, String sourceHostname, String mode) 
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status && modHasChannel(i,channel))
        		objMods[i].onMode(channel, sourceNick, sourceLogin, sourceHostname, mode);
    	}
    }
	@Override
	public final void onNickChange(String oldNick, String login, String hostname, String newNick) 
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status)
        		objMods[i].onNickChange(oldNick, login, hostname, newNick);
    	}
    }
    @Override
    public final void onNotice(String sourceNick, String sourceLogin, String sourceHostname, String target, String notice)  
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status)
        		objMods[i].onNotice(sourceNick, sourceLogin, sourceHostname, target, notice);
    	}
    }
    @Override
    public final void onOp(String channel, String sourceNick, String sourceLogin, String sourceHostname, String recipient) 
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status && modHasChannel(i,channel))
        		objMods[i].onOp(channel, sourceNick, sourceLogin, sourceHostname, recipient);
    	}
    }
    @Override
    public final void onPart(String channel, String sender, String login, String hostname)  
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status && modHasChannel(i,channel))
        		objMods[i].onPart(channel, sender, login, hostname);
    	}
    ...
    public final void onPing(String sourceNick, String sourceLogin, String sourceHostname, String target, String pingValue)
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status)
        		objMods[i].onPing(sourceNick, sourceLogin, sourceHostname, target, pingValue);
    	}
    }
    @Override
    public final void onPrivateMessage(String sender, String login, String hostname, String message) 
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status)
        		objMods[i].onPrivateMessage(sender, login, hostname, message);
    	}
    }
    @Override
    public final void onQuit(String sourceNick, String sourceLogin, String sourceHostname, String reason) 
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status)
        		objMods[i].onQuit(sourceNick, sourceLogin, sourceHostname, reason);
    	}
    }
    @Override
    public final void onRemoveChannelBan(String channel, String sourceNick, String sourceLogin, String sourceHostname, String hostmask)
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status && modHasChannel(i,channel))
        		objMods[i].onRemoveChannelBan(channel, sourceNick, sourceLogin, sourceHostname, hostmask);
    	}
    }
    @Override
    public final void onRemoveChannelKey(String channel, String sourceNick, String sourceLogin, String sourceHostname, String key) 
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status && modHasChannel(i,channel))
        		objMods[i].onRemoveChannelKey(channel, sourceNick, sourceLogin, sourceHostname, key);
    	}
    }
    @Override
    public final void onRemoveChannelLimit(String channel, String sourceNick, String sourceLogin, String sourceHostname) 
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status && modHasChannel(i,channel))
        		objMods[i].onRemoveChannelLimit(channel, sourceNick, sourceLogin, sourceHostname);
    	}
    }
    @Override
    public final void onRemoveInviteOnly(String channel, String sourceNick, String sourceLogin, String sourceHostname) 
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status && modHasChannel(i,channel))
        		objMods[i].onRemoveInviteOnly(channel, sourceNick, sourceLogin, sourceHostname);
    	}
    }
    @Override
    public final void onRemoveModerated(String channel, String sourceNick, String sourceLogin, String sourceHostname)  
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status && modHasChannel(i,channel))
        		objMods[i].onRemoveModerated(channel, sourceNick, sourceLogin, sourceHostname);
    	}
    }
    @Override
    public final void onRemoveNoExternalMessages(String channel, String sourceNick, String sourceLogin, String sourceHostname)
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status && modHasChannel(i,channel))
        		objMods[i].onRemoveNoExternalMessages(channel, sourceNick, sourceLogin, sourceHostname);
    	}
    }
    @Override
    public final void onRemovePrivate(String channel, String sourceNick, String sourceLogin, String sourceHostname)
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status && modHasChannel(i,channel))
        		objMods[i].onRemovePrivate(channel, sourceNick, sourceLogin, sourceHostname); 
    	}
    }
    @Override
    public final void onRemoveSecret(String channel, String sourceNick, String sourceLogin, String sourceHostname)
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status && modHasChannel(i,channel))
        		objMods[i].onRemoveSecret(channel, sourceNick, sourceLogin, sourceHostname);
    	}
    }
    @Override
    public final void onRemoveTopicProtection(String channel, String sourceNick, String sourceLogin, String sourceHostname)
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status && modHasChannel(i,channel))
        		objMods[i].onRemoveTopicProtection(channel, sourceNick, sourceLogin, sourceHostname);
    	}
    }
    @Override
    public final void onServerPing(String response) 
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status)
        		objMods[i].onServerPing(response);
    	}
    }
    @Override
    public final void onServerResponse(int code, String response)
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status)
        		objMods[i].onServerResponse(code, response);
    	}
    }
    @Override
    public final void onSetChannelBan(String channel, String sourceNick, String sourceLogin, String sourceHostname, String hostmask)
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status && modHasChannel(i,channel))
        		objMods[i].onSetChannelBan(channel, sourceNick, sourceLogin, sourceHostname, hostmask);
    	}
    }
    @Override
    public final void onSetChannelKey(String channel, String sourceNick, String sourceLogin, String sourceHostname, String key)
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status && modHasChannel(i,channel))
        		objMods[i].onSetChannelKey(channel, sourceNick, sourceLogin, sourceHostname, key);
    	}
    }
    @Override
    public final void onSetChannelLimit(String channel, String sourceNick, String sourceLogin, String sourceHostname, int limit)
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status && modHasChannel(i,channel))
        		objMods[i].onSetChannelLimit(channel, sourceNick, sourceLogin, sourceHostname, limit);
    	}
    }
    @Override
    public final void onSetInviteOnly(String channel, String sourceNick, String sourceLogin, String sourceHostname) 
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status && modHasChannel(i,channel))
        		objMods[i].onSetInviteOnly(channel, sourceNick, sourceLogin, sourceHostname);
    	}
    }
    @Override
    public final void onSetModerated(String channel, String sourceNick, String sourceLogin, String sourceHostname) 
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status && modHasChannel(i,channel))
        		objMods[i].onSetModerated( channel, sourceNick, sourceLogin, sourceHostname);
    	}
    }
    @Override
    public final void onSetNoExternalMessages(String channel, String sourceNick, String sourceLogin, String sourceHostname) 
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status && modHasChannel(i,channel))
        		objMods[i].onSetNoExternalMessages(channel, sourceNick, sourceLogin, sourceHostname);
    	}
    }
    @Override
    public final void onSetPrivate(String channel, String sourceNick, String sourceLogin, String sourceHostname) 
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status && modHasChannel(i,channel))
        		objMods[i].onSetPrivate(channel, sourceNick, sourceLogin, sourceHostname);
    	}
    }
    @Override
    public final void onSetSecret(String channel, String sourceNick, String sourceLogin, String sourceHostname) 
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status && modHasChannel(i,channel))
        		objMods[i].onSetSecret(channel, sourceNick, sourceLogin, sourceHostname);
    	}
    }
    @Override
    public final void onSetTopicProtection(String channel, String sourceNick, String sourceLogin, String sourceHostname) 
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status && modHasChannel(i,channel))
        		objMods[i].onSetTopicProtection(channel, sourceNick, sourceLogin, sourceHostname);
    	}
    }
    @Override
    public final void onTime(String sourceNick, String sourceLogin, String sourceHostname, String target)  
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status)
        		objMods[i].onTime(sourceNick, sourceLogin, sourceHostname, target);
    	}
    }   
    @SuppressWarnings("deprecation")
	@Override
	public final void onTopic(String channel, String topic) 
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status && modHasChannel(i,channel))
        		objMods[i].onTopic(channel, topic);
    	}
    }
    @Override
    public final void onTopic(String channel, String topic, String setBy, long date, boolean changed) 
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status && modHasChannel(i,channel))
        		objMods[i].onTopic(channel, topic, setBy, date, changed);
    	}
    }
    @Override
    ...
    public final void onUserList(String channel, User[] users) 
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status && modHasChannel(i,channel))
        		objMods[i].onUserList(channel, users);
    	}
    }
    @Override
    public final void onUserMode(String targetNick, String sourceNick, String sourceLogin, String sourceHostname, String mode) 
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status)
        		objMods[i].onUserMode( targetNick, sourceNick, sourceLogin, sourceHostname, mode);
    	}
    }
    @Override
    public final void onVersion(String sourceNick, String sourceLogin, String sourceHostname, String target) 
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status)
        		objMods[i].onVersion(sourceNick, sourceLogin, sourceHostname, target);
    	}
    }
    @Override
	public void onVoice(String channel, String sourceNick, String sourceLogin, String sourceHostname, String recipient) 
    {
    	...
    	for( int i = 0; i < objMods.length; i++ ){
    		if(objMods[i].status && modHasChannel(i,channel))
        		objMods[i].onVoice(channel, sourceNick, sourceLogin, sourceHostname, recipient);
    	}
    }
}
